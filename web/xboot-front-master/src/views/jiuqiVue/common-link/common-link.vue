...
<!--
                                <queryDepartmenttree @on-change="handleSelectDepTree" @func="getDepTitle" ref="depTree"></queryDepartmenttree>
-->
                ...
                <Form-item label="手机号" prop="mobile">
                  ...
                  />
                    ...
                    <Row class="operation">
                        ...
                        <Button @click="doPost">link测试</Button>
...
<script>
    ...
    import {fromPost} from "@/libs/tool.js";
    ...
    import queryDepartmenttree from "@/views/jiuqiVue/myTemplate/queryDepartment-tree.vue";
        ...
        methods: {
            ...
            getRoleList() {
              ...
              /*  this.subsystemNews.title="test";
                ...
                getCommonLink(this.queryForm).then(res => {
                    this.loading = false;
                    if (res.success == true) {
                ...
                this.$refs.editSubsystemNews.validate(valid => {
                    if (valid) {
                        if (this.modalType == 0) {
                            ...
                            addCommonLink(this.editSubsystemNews).then(res => {
            ...
            submitBJ() {
                        ...
                        /**
                         * 注意：
                         * 此处的部门已经在方法 editDepTitle（）中通过子组件赋值完成，
                         * 所以此处不需要在赋值，如果赋值的话会导致修改不成功（会将原来的值重新赋回去）
                         * */
                        //this.BJInfo.department=this.newXQ.department;
                        this.BJInfo.url=this.newXQ.url;
                        editCommonLink(this.BJInfo).then(res => {
                            this.submitLoading = false;
                            if (res.success == true) {
                                this.$Message.success("操作成功");
                                this.getRoleList();
            ...
            editDep(v) {
                ...
                //njp 20190715 通过id查询链接详情 begin
                getCommonLinkById(v.id).then(res => {
                    this.operationLoading = false;
                    if (res.success == true) {
                        this.newXQ = res.result;
                        this.BJInfo.department=this.newXQ.department;
            ...
            doPost() {  // to:提交动作（action）,p:参数
                ...
                //创建input begin
                var myInputUserCode = document.createElement("input");
                var myInputTemplateName = document.createElement("input");
                var test3 = document.createElement("input");
                myInputUserCode.setAttribute("name", "userCode");  // 为input对象设置name

                myInputUserCode.setAttribute("value", "VwzQZnLrfgOyTMNVbm67K5F6n4QwiFVm");  // 为input对象设置value


                test3.setAttribute("name", "changingtype");  // 为input对象设置name

                test3.setAttribute("value", "todo");  // 为input对象设置value

                myInputTemplateName.setAttribute("name", "params");  // 为input对象设置name

                myInputTemplateName.setAttribute("value", "4E10B31B19D8F4AA62C38F426EF28A4E@6CD600B84000000165B03F92BBA99561@5C96D7CA4E4F7D7330AAD6452EE9FE41@6C659EBB400000017C0E6E6F87838ED8@0000016CD600E4FE22126BD6AC2A2469@true");  // 为input对象设置value

                myForm.appendChild(myInputUserCode);
                myForm.appendChild(test3);
                myForm.appendChild(myInputTemplateName);
